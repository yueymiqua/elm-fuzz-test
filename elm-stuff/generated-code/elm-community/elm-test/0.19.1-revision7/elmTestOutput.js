...
var $author$project$TestExample$addOne = function (x) {
	return 1 + x;
};
...
var $elm_explorations$test$Test$Internal$Only = function (a) {
	return {__elmTestSymbol: __elmTestSymbol, $: 'Only', a: a};
};
var $elm_explorations$test$Test$fuzzWithHelp = F2(
	function (options, aTest) {
		switch (aTest.$) {
			case 'UnitTest':
				return aTest;
			case 'FuzzTest':
				var run = aTest.a;
				return $elm_explorations$test$Test$Internal$FuzzTest(
					F2(
						function (seed, _v1) {
							return A2(run, seed, options.runs);
						}));
			case 'Labeled':
				var label = aTest.a;
				var subTest = aTest.b;
				return A2(
					$elm_explorations$test$Test$Internal$Labeled,
					label,
					A2($elm_explorations$test$Test$fuzzWithHelp, options, subTest));
			case 'Skipped':
				var subTest = aTest.a;
				return $elm_explorations$test$Test$Internal$Only(
					A2($elm_explorations$test$Test$fuzzWithHelp, options, subTest));
			case 'Only':
				var subTest = aTest.a;
				return $elm_explorations$test$Test$Internal$Only(
			...
			default:
				var tests = aTest.a;
				return $elm_explorations$test$Test$Internal$Batch(
					A2(
						$elm$core$List$map,
						$elm_explorations$test$Test$fuzzWithHelp(options),
						tests));
		}
	});
var $elm$core$Basics$lt = _Utils_lt;
var $elm_explorations$test$Test$fuzzWith = F4(
	function (options, fuzzer, desc, getTest) {
		return (options.runs < 1) ? $elm_explorations$test$Test$Internal$failNow(
			{
				description: 'Fuzz tests must have a run count of at least 1, not ' + ($elm$core$String$fromInt(options.runs) + '.'),
				reason: $elm_explorations$test$Test$Runner$Failure$Invalid($elm_explorations$test$Test$Runner$Failure$NonpositiveFuzzCount)
			}) : A2(
			$elm_explorations$test$Test$fuzzWithHelp,
			options,
			A3($elm_explorations$test$Test$fuzz, fuzzer, desc, getTest));
	});
...
var $author$project$TestExample$addOneTests = A2(
	$elm_explorations$test$Test$describe,
	'addOne',
	_List_fromArray(
		[
			A4(
			$elm_explorations$test$Test$fuzzWith,
			{runs: 200},
			...
			function (num) {
				return A2(
					...
					num + 1,
					$author$project$TestExample$addOne(num));
			})
		]));
...
var $author$project$Test$Generated$Main$main = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$author$project$Test$Runner$Node$check($author$project$TestExample$addOneTests)
				]))
		]));
_Platform_export({'Test':{'Generated':{'Main':{'init':$author$project$Test$Generated$Main$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-77941.sock";
var net = require('net'),
